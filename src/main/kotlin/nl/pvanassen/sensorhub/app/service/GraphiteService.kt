...
class GraphiteService(graphiteClient: GraphiteClient) {
    ...
    private val store = LinkedMultiValueMap<Long, TimedSensor<Sensor>>()
    ...
    private fun getAndClear(timestamp: Long): List<TimedSensor<Sensor>> {
        return storeLock.withLock {
            val cache = store.values
                    .flatten()
                    .groupingBy { "${it.timestamp}.${it.name}" }
                    .reduce { _, sensor1, sensor2 ->
                        maxOf(sensor1, sensor2, Comparator<TimedSensor<Sensor>> { s1, s2 -> (s2.timestamp - s1.timestamp).toInt() }) }
                    .values
    ...
    fun storeStats(namedSensor: Mono<NamedSensor<Sensor>>): Mono<Boolean> {
        return namedSensor
                .map { TimedSensor<Sensor>(it, getTimestamp()) }
                .map { store(it) }
                .map { true}
    }
    ...
    private fun store(timedSensor: TimedSensor<Sensor>) {
        storeLock.withLock {
